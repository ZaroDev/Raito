...
void main() {
    vec3 albedo = pow(texture(u_Albedo, TexCoords).rgb, vec3(GAMMA));
    float roughness = texture(u_MetalRoughness, TexCoords).g;
    float metallic = texture(u_MetalRoughness, TexCoords).b;
    float occlusion = texture(u_SSAO, TexCoords).r;

    vec3 N = fromNormalMap();
...

    {
        ...
        float vis = shadowCalculationDirectional(WorldPos, directionalDir, N);
    ...
    for(int i = 0; i < u_PointSize; i++){
        ...
        // calculate per-light radiance
        vec3 L = normalize(lighPos - WorldPos);
        vec3 H = normalize(V + L);
        float distance = length(lighPos - WorldPos);
    ...
    BrightColor = pow(texture(u_Emissive, TexCoords), vec4(GAMMA));
}