...
namespace Raito::Renderer::D3D12::Upload
{
	namespace
	{
		std::vector<nvrhi::BufferHandle> g_Meshes{};
	...
	u32 AddMesh(Assets::Mesh* mesh)
	{
		...
		const auto& vertexBuffer = g_Meshes.emplace_back(Core::NVDevice()->createBuffer(vertexBufferDesc));

		const nvrhi::CommandListHandle commandList = Core::GraphicsCommand().CommandList();
		commandList->open();
		commandList->writeBuffer(vertexBuffer, mesh->Vertices.data(), mesh->Vertices.size() * sizeof(Assets::Vertex));
		commandList->close();
	...
	u32 AddTexture(Assets::Texture* texture, ubyte* data)
	{
		...
		const nvrhi::CommandListHandle& commandList = Core::GraphicsCommand().CommandList();
		commandList->open();
		commandList->writeTexture(textureH, 0, 0, data, 0);
		commandList->close();
		Core::NVDevice()->executeCommandList(commandList);
